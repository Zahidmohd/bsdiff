...
# Measure performance, memory usage, and patch size for each test file
for i, size in enumerate(sizes):
    ...
    # Measure memory usage before creating patch
    mem_before_creation = get_memory_usage()
    print(f"Memory usage before patch creation: {mem_before_creation / 1024 / 1024:.2f} MB")
    
    # Measure patch creation
    creation_time, patch_memory = create_patch_mmap(old_file, new_file, patch_file)
    ...
    # Measure memory usage after patch creation
    mem_after_creation = get_memory_usage()
    print(f"Memory usage after patch creation: {mem_after_creation / 1024 / 1024:.2f} MB")
    ...
    # Measure memory usage after patch application
    mem_after_application = get_memory_usage()
    print(f"Memory usage after patch application: {mem_after_application / 1024 / 1024:.2f} MB")
